...
namespace tchecker {
  ...
  namespace parsing {
    ...
    // parses from a buffer instead of a file
    std::shared_ptr<tchecker::parsing::system_declaration_t>
    parse_system_declaration_from_buffer(std::string const & buffer,
                                   std::string const & filename)
    {
      std::size_t old_error_count = tchecker::log_error_count();

      YY_BUFFER_STATE state = spyy_scan_bytes(buffer.data(), buffer.size());
