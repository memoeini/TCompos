...
/*!< Place holder integer variable set container, should stay empty */
static std::vector<std::pair<tchecker::variable_id_t, tchecker::variable_id_t>> place_holder_intvar_set;

/*!< Place holder integer variable guard container, should stay empty */
static std::vector<unsigned> place_holder_intvarconstr;
                                 ...
                                 tchecker::ta_ha::initial_value_t const & initial_range)
{
  ...
  // check invariant
  tchecker::vm_ha::vm_t & vm = system.vm();
  for (tchecker::loc_id_t loc_id : *vloc) {
    if (vm.run(system.invariant_bytecode(loc_id), *intval, invariant, place_holder_intvarconstr, place_holder_clkreset, place_holder_intvar_set) == 0)
...
{
  tchecker::ta_ha::final_iterator_t it{tchecker::syncprod::final_edges(system.as_syncprod_system(), labels),
                              ...
                              tchecker::ta_ha::outgoing_edges_value_t const & edges)
{
  ...
  for (tchecker::loc_id_t loc_id : *vloc) {
    if (vm.run(system.invariant_bytecode(loc_id), *intval, src_invariant, place_holder_intvarconstr, place_holder_clkreset, place_holder_intvar_set) == 0)
      return tchecker::STATE_INTVARS_SRC_INVARIANT_VIOLATED;
    assert(place_holder_clkreset.empty());
  }
  ...
  for (tchecker::system::edge_const_shared_ptr_t const & edge : edges) {
    if (vm.run(system.guard_bytecode(edge->id()), *intval, guard, intvarconstr, place_holder_clkreset, place_holder_intvar_set) == 0)
  ...
  // apply statements
  for (tchecker::system::edge_const_shared_ptr_t const & edge : edges) {
    if (vm.run(system.statement_bytecode(edge->id()), *intval, place_holder_clkconstr, place_holder_intvarconstr, reset, intvar_set) == 0)
  ...
  for (tchecker::loc_id_t loc_id : *vloc) {
    if (vm.run(system.invariant_bytecode(loc_id), *intval, tgt_invariant, place_holder_intvarconstr, place_holder_clkreset, place_holder_intvar_set) == 0)
      return tchecker::STATE_INTVARS_TGT_INVARIANT_VIOLATED;
    assert(place_holder_clkreset.empty());
  }
...
{
  tchecker::ta_ha::incoming_edges_iterator_t it{
...
tchecker::state_status_t prev(tchecker::ta_ha::system_t const & system,
                              ...
                              tchecker::intrusive_shared_ptr_t<tchecker::shared_vedge_t> const & vedge,
                              tchecker::clock_constraint_container_t & src_invariant,
                              tchecker::clock_constraint_container_t & guard, std::vector<unsigned> & intvarconstr,
                              tchecker::clock_reset_container_t & reset,
                              std::vector<std::pair<tchecker::variable_id_t, tchecker::variable_id_t>> & intvar_set,
                              tchecker::clock_constraint_container_t & tgt_invariant,
                              tchecker::ta_ha::incoming_edges_value_t const & v)
{
  ...
  tchecker::state_status_t status =
      tchecker::ta_ha::next(system, vloc, intval, vedge, src_invariant, guard, intvarconstr, reset, intvar_set, tgt_invariant, edges);
  if (status != tchecker::STATE_OK)
    return status;
...
{
  return tchecker::syncprod::is_initial(system.as_syncprod_system(), s) && tchecker::ta_ha::is_initial(system, s.intval());
}
...
ta_t::ta_t(std::shared_ptr<tchecker::ta_ha::system_t const> const & system, enum tchecker::ts::sharing_type_t sharing_type,
...
tchecker::ta_ha::initial_range_t ta_t::initial_edges() { return tchecker::ta_ha::initial_edges(*_system); }

void ta_t::initial(tchecker::ta_ha::initial_value_t const & init_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::ta::state_sptr_t s = _state_allocator.construct();
  tchecker::ta_ha::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::ta_ha::initial(*_system, *s, *t, init_edge);
  if (status & mask) {
...
tchecker::ta_ha::outgoing_edges_range_t ta_t::outgoing_edges(tchecker::ta::const_state_sptr_t const & s)
{
  return tchecker::ta_ha::outgoing_edges(*_system, s->vloc_ptr());
}

void ta_t::next(tchecker::ta::const_state_sptr_t const & s, tchecker::ta_ha::outgoing_edges_value_t const & out_edge,
                std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::ta::state_sptr_t nexts = _state_allocator.clone(*s);
  tchecker::ta_ha::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::ta_ha::next(*_system, *nexts, *t, out_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(nexts);
      share(t);
...
final_range_t ta_t::final_edges(boost::dynamic_bitset<> const & labels) { return tchecker::ta_ha::final_edges(*_system, labels); }

void ta_t::final(final_value_t const & final_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::ta::state_sptr_t s = _state_allocator.construct();
  tchecker::ta_ha::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::ta_ha::final(*_system, *s, *t, final_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(s);
      share(t);
...
{
  return tchecker::ta_ha::incoming_edges(*_system, s->vloc_ptr());
}
                ...
                tchecker::state_status_t mask)
{
  ...
  tchecker::ta_ha::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::ta_ha::prev(*_system, *prevs, *t, in_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(prevs);
      share(t);
...
void ta_t::build(std::map<std::string, std::string> const & attributes, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::ta::state_sptr_t s = _state_allocator.construct();
  tchecker::ta_ha::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::ta_ha::initialize(*_system, *s, *t, attributes);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(s);
      share(t);
...
{
  return tchecker::ta_ha::labels(*_system, *s);
}

void ta_t::attributes(tchecker::ta::const_state_sptr_t const & s, std::map<std::string, std::string> & m) const
{
  tchecker::ta_ha::attributes(*_system, *s, m);
}

void ta_t::attributes(tchecker::ta_ha::const_transition_sptr_t const & t, std::map<std::string, std::string> & m) const
{
  tchecker::ta_ha::attributes(*_system, *t, m);
}

bool ta_t::is_valid_final(tchecker::ta::const_state_sptr_t const & s) const
{
  return tchecker::ta_ha::is_valid_final(*_system, *s);
}

bool ta_t::is_initial(tchecker::ta::const_state_sptr_t const & s) const { return tchecker::ta_ha::is_initial(*_system, *s); }

// Sharing

void ta_t::share(tchecker::ta::state_sptr_t & s) { _state_allocator.share(s); }

void ta_t::share(tchecker::ta_ha::transition_sptr_t & t) { _transition_allocator.share(t); }
