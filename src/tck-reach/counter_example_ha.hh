...
#ifndef TCHECKER_TCK_REACH_COUNTER_EXAMPLE_HA_HH
#define TCHECKER_TCK_REACH_COUNTER_EXAMPLE_HA_HH
 ...
 */
template <class GRAPH> tchecker::zg_ha::path::symbolic::finite_path_t * symbolic_counter_example_zg(GRAPH const & g)
{
  std::shared_ptr<tchecker::zg_ha::zg_t> zg{tchecker::zg_ha::factory(
      g.zg().system_ptr(), g.zg().sharing_type(), tchecker::zg::STANDARD_SEMANTICS, tchecker::zg_ha::NO_EXTRAPOLATION, 128, 128)};
  ...
  if (!found)
    return new tchecker::zg_ha::path::symbolic::finite_path_t{zg};
  ...
  return tchecker::zg_ha::path::symbolic::compute_finite_path(zg, initial_vloc, vedge_seq, true);
}
 ...
 */
template <class GRAPH> tchecker::zg_ha::path::concrete::finite_path_t * concrete_counter_example_zg(GRAPH const & g)
{
  std::unique_ptr<tchecker::zg_ha::path::symbolic::finite_path_t> symbolic_cex{
      tchecker::tck_reach::symbolic_counter_example_zg<GRAPH>(g)};

  // Compute concrete counter-exemple from symbolic counter-example
  tchecker::zg_ha::path::concrete::finite_path_t * cex = tchecker::zg_ha::path::concrete::compute_finite_path(*symbolic_cex);
