...
edge_t::edge_t(tchecker::vedge_sptr_t const & vedge, tchecker::clock_constraint_container_t guards)
   : tchecker::graph::edge_vedge_t(vedge), tchecker::graph::edge_rel_transition(guards)
{
}
...
run(tchecker::zg_ha::zg_t & zg, std::shared_ptr<tchecker::ta::system_t const> & env,
   ...
   int num_int_vars, long long int iteration_num)
{
 ...
 for (auto && [status, s, t] : sst) {
   ...
   auto vlocc = to_string(initial_node->state_ptr()->vloc(), graph_system);
 ...
 while (!waiting->empty()) {
   ...
   auto vlocc = to_string(node->state_ptr()->vloc(), graph_system);
   ...
   if (accepting(node, zg, labels)) {
     ...
     ispinode = true;
     sst.clear();
     continue;      // continue with the next node, no need to advance this node further
   }
   ...
   for (typename tchecker::zg_ha::zg_t::outgoing_edges_value_t && out_edge : out_edges)
     zg.next(node_state, out_edge, sst);      // fill sst
   ...
   for (auto && [status, s, t] : sst) {
     ...
     to_string(new_reset_clock_history, resetv_string);
     ...
     graph.add_edge(node, next_node, t->vedge_ptr(), t->guard_container());
   ...
   long long int iteration_num)
{
 ...
 std::shared_ptr<tchecker::ta::system_t const> env{new tchecker::ta::system_t{*envdecl}};
 ...
 std::shared_ptr<tchecker::zg_ha::zg_t> zg{tchecker::zg_ha::factory(
     system, tchecker::ts::SHARING, tchecker::zg::ELAPSED_SEMANTICS, tchecker::zg::EXTRA_LU_PLUS_GLOBAL, block_size, table_size)};
