...
{
  // return tchecker::tck_reach::symbolic_counter_example_zg<tchecker::tck_reach::zg_reach_compos::graph_t>(g);
}
...
{
  // return tchecker::tck_reach::concrete_counter_example_zg<tchecker::tck_reach::zg_reach_compos::graph_t>(g);
}
                                         ...
                                         enum tchecker::waiting::policy_t policy)
{
  ...
  while (!waiting->empty()) {
    ...
    ++counter;
    if (counter % 100000 == 0) {
      std::cout << "The size of the waiting queue is increasing | " << graph.nodes_count() << std::endl;
    }
    ...
    std::string const & search_order, std::size_t block_size, std::size_t table_size)
{
  ...
  std::shared_ptr<tchecker::zg_compos::zg_t> zg{tchecker::zg_compos::factory(original_system, system, tchecker::ts::SHARING, tchecker::zg::ELAPSED_SEMANTICS,
                                                                             tchecker::zg::EXTRA_LU_PLUS_GLOBAL, block_size, table_size)};
