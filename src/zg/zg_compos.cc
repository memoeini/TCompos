...
void zg_t::initial(tchecker::zg_compos::initial_value_t const & init_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_compos::initial(*_system, *s, *t, *_semantics, *_extrapolation, init_edge);
  if (status & mask) {
                ...
                std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_compos::next(*_system, *nexts, *nextt, *_semantics, *_extrapolation, out_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(nexts);
      share(nextt);
...
void zg_t::final(final_value_t const & final_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::zg::state_sptr_t s = _state_allocator.construct();
  tchecker::zg::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::zg_compos::final(*_system, *s, *t, *_semantics, *_extrapolation, final_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(s);
      share(t);
                ...
                tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_compos::prev(*_system, *prevs, *prevt, *_semantics, *_extrapolation, in_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(prevs);
      share(prevt);
                             ...
                             std::size_t table_size)
{
  std::shared_ptr<tchecker::zg::extrapolation_t> extrapolation{
      tchecker::zg::extrapolation_factory(extrapolation_type, *original_system)};       // send original system as a reference to speed up extrapolation
