...
tchecker::state_status_t initial(tchecker::ta::system_t const & system,
                                 ...
                                 tchecker::zg_compos::extrapolation_t & extrapolation,
                                 tchecker::zg_compos::initial_value_t const & initial_range)
{
...
tchecker::state_status_t final(tchecker::ta::system_t const & system,
                               ...
                               tchecker::intrusive_shared_ptr_t<tchecker::shared_vedge_t> const & vedge,
                               tchecker::clock_constraint_container_t & invariant, tchecker::zg::semantics_t & semantics,
                               tchecker::zg_compos::extrapolation_t & extrapolation, tchecker::zg_compos::final_value_t const & final_range)
{
...
tchecker::state_status_t next(tchecker::ta::system_t const & system,
                              ...
                              tchecker::zg_compos::extrapolation_t & extrapolation, tchecker::zg_compos::outgoing_edges_value_t const & edges)
{
...
tchecker::state_status_t prev(tchecker::ta::system_t const & system,
                              ...
                              tchecker::clock_constraint_container_t & tgt_invariant, tchecker::zg::semantics_t & semantics,
                              tchecker::zg_compos::extrapolation_t & extrapolation, tchecker::zg_compos::incoming_edges_value_t const & edges)
{
  bool tgt_delay_allowed = tchecker::ta::delay_allowed(system, *vloc);
...
{
  return tchecker::ta::is_initial(system, s) && tchecker::zg_compos::is_initial(system, s.zone());
}
...
zg_t::zg_t(std::shared_ptr<tchecker::ta::system_t const> const & system, enum tchecker::ts::sharing_type_t sharing_type,
           ...
           std::shared_ptr<tchecker::zg_compos::extrapolation_t> const & extrapolation, std::size_t block_size, std::size_t table_size)
...
initial_range_t zg_t::initial_edges() { return tchecker::zg_compos::initial_edges(*_system); }

void zg_t::initial(tchecker::zg_compos::initial_value_t const & init_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::zg::state_sptr_t s = _state_allocator.construct();
  tchecker::zg::transition_sptr_t t = _transition_allocator.construct();

  tchecker::state_status_t status = tchecker::zg_compos::initial(*_system, s->vloc_ptr(), s->intval_ptr(), s->zone_ptr(), t->vedge_ptr(), t->src_invariant_container(),
...
tchecker::zg_compos::outgoing_edges_range_t zg_t::outgoing_edges(tchecker::zg::const_state_sptr_t const & s)
{
  return tchecker::zg_compos::outgoing_edges(*_system, s->vloc_ptr());
}

void zg_t::next(tchecker::zg::const_state_sptr_t const & s, tchecker::zg_compos::outgoing_edges_value_t const & out_edge,
                std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_compos::next(*_system, nexts->vloc_ptr(), nexts->intval_ptr(), nexts->zone_ptr(), nextt->vedge_ptr(), nextt->src_invariant_container(),
...
final_range_t zg_t::final_edges(boost::dynamic_bitset<> const & labels) { return tchecker::zg_compos::final_edges(*_system, labels); }
...
void zg_t::final(final_value_t const & final_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_compos::final(*_system, s->vloc_ptr(), s->intval_ptr(), s->zone_ptr(), t->vedge_ptr(), t->src_invariant_container(),
...
{
  return tchecker::zg_compos::incoming_edges(*_system, s->vloc_ptr());
}
                ...
                tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_compos::prev(*_system, prevs->vloc_ptr(), prevs->intval_ptr(), prevs->zone_ptr(), prevt->vedge_ptr(), prevt->src_invariant_container(),
...
void zg_t::build(std::map<std::string, std::string> const & attributes, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::zg::state_sptr_t s = _state_allocator.construct();
  tchecker::zg::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::zg_compos::initialize(*_system, *s, *t, attributes);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(s);
      share(t);
...
{
  return tchecker::zg_compos::labels(*_system, *s);
}

void zg_t::attributes(tchecker::zg::const_state_sptr_t const & s, std::map<std::string, std::string> & m) const
{
  tchecker::zg_compos::attributes(*_system, *s, m);
}

void zg_t::attributes(tchecker::zg::const_transition_sptr_t const & t, std::map<std::string, std::string> & m) const
{
  tchecker::zg_compos::attributes(*_system, *t, m);
}

bool zg_t::is_valid_final(tchecker::zg::const_state_sptr_t const & s) const
{
  return tchecker::zg_compos::is_valid_final(*_system, *s);
}

bool zg_t::is_initial(tchecker::zg::const_state_sptr_t const & s) const { return tchecker::zg_compos::is_initial(*_system, *s); }
...
tchecker::zg::state_sptr_t initial(tchecker::zg_compos::zg_t & zg, tchecker::vloc_t const & vloc, tchecker::state_status_t mask)
{
  std::vector<tchecker::zg_compos::zg_t::sst_t> v;
  zg.initial(v, mask);
...
std::tuple<tchecker::zg::state_sptr_t, tchecker::zg::transition_sptr_t> next(tchecker::zg_compos::zg_t & zg,
...
tchecker::zg_compos::zg_t * factory(std::shared_ptr<tchecker::ta::system_t const> const & original_system,
                             std::shared_ptr<tchecker::ta::system_t const> const & system,
                             enum tchecker::ts::sharing_type_t sharing_type, enum tchecker::zg::semantics_type_t semantics_type,
                             enum tchecker::zg_compos::extrapolation_type_t extrapolation_type, std::size_t block_size,
                             std::size_t table_size)
{
  std::shared_ptr<tchecker::zg_compos::extrapolation_t> extrapolation{
      tchecker::zg_compos::extrapolation_factory(extrapolation_type, *original_system, *system)};
  ...
  return new tchecker::zg_compos::zg_t(system, sharing_type, semantics, extrapolation, block_size, table_size);
}

tchecker::zg_compos::zg_t * factory(std::shared_ptr<tchecker::ta::system_t const> const & system,
                             enum tchecker::ts::sharing_type_t sharing_type, enum tchecker::zg::semantics_type_t semantics_type,
                             enum tchecker::zg_compos::extrapolation_type_t extrapolation_type,
                             tchecker::clockbounds::clockbounds_t const & clock_bounds, std::size_t block_size,
                             std::size_t table_size)
{
  std::shared_ptr<tchecker::zg_compos::extrapolation_t> extrapolation{
      tchecker::zg_compos::extrapolation_factory(extrapolation_type, clock_bounds)};
  if (extrapolation.get() == nullptr)
    return nullptr;
  std::shared_ptr<tchecker::zg::semantics_t> semantics{tchecker::zg::semantics_factory(semantics_type)};
  return new tchecker::zg_compos::zg_t(system, sharing_type, semantics, extrapolation, block_size, table_size);
