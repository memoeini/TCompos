                              ...
                              tchecker::zg_ha::outgoing_edges_value_t const & sync_edges)
{
  ...
  // std::cout << "Guard size before VM: " << guard.size() << std::endl;
  ...
  // std::cout << "Guard size after VM: " << guard.size() << std::endl;
...
{
  return tchecker::ta::is_initial(system, s) && tchecker::zg_ha::is_initial(system, s.zone());
}
...
void zg_t::initial(tchecker::zg_ha::initial_value_t const & init_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_ha::initial(*_system, *s, *t, *_semantics, *_extrapolation, init_edge);
  if (status & mask) {
                ...
                std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_ha::next(*_system, *nexts, *nextt, *_semantics, *_extrapolation, out_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(nexts);
      share(nextt);
...
void zg_t::final(final_value_t const & final_edge, std::vector<sst_t> & v, tchecker::state_status_t mask)
{
  tchecker::zg::state_sptr_t s = _state_allocator.construct();
  tchecker::zg::transition_sptr_t t = _transition_allocator.construct();
  tchecker::state_status_t status = tchecker::zg_ha::final(*_system, *s, *t, *_semantics, *_extrapolation, final_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(s);
      share(t);
                ...
                tchecker::state_status_t mask)
{
  ...
  tchecker::state_status_t status = tchecker::zg_ha::prev(*_system, *prevs, *prevt, *_semantics, *_extrapolation, in_edge);
  if (status & mask) {
    if (_sharing_type == tchecker::ts::SHARING) {
      share(prevs);
      share(prevt);
...
std::tuple<tchecker::zg::state_sptr_t, tchecker::zg::transition_sptr_t> next(tchecker::zg_ha::zg_t & zg,
